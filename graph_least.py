...
def plot_disc_graph_least(values, ax):
    ...
    # Transform values to match form grid (20-0 scale, inverted)
    mapped_values = [20 - (v * 2) for v in values]  # Invert the values for LEAST graph
    ...
    # Add all grid numbers from 0 to 20 (inverted display)
    for i in range(21):
        # Draw light gray horizontal grid lines
        alpha = 0.3 if i % 4 == 0 else 0.15
        ...
        if i % 2 == 0:  # Show even numbers
            ax.text(-0.6, i, str(20-i), ha='right', va='center', fontsize=6)
    ...
    segments = ['H', 'MH', 'M', 'ML', 'L']  # Inverted for LEAST graph
    y_positions = [2, 6, 10, 14, 18]  # Adjusted for inverted scale
    ...
    ax.set_title("Graph 2 LEAST\nCore, Private Self", pad=10, fontsize=8, fontweight='bold')
